...
# Read data as a character vector of dataframe
bin_diag <- readr::read_table(
  "data-raw/binary-diagnostic.txt",
  col_names = "bin",
  col_types = "c")

# Create vector with names that are going to be assigned to each digit in the
# binary sequence
digits <- paste0("b", 1:12)

# Separate each binary sequence into single values as separate variables
bin_diag_sep <- bin_diag %>%
  tidyr::separate(
    bin,
    sep = 1:11,
    into = digits)

# Calculate gamma in binary format
gamma_binary <- purrr::map(
  digits,
  ~ modeest::mfv(bin_diag_sep[[.x]])
) %>%
  unlist() %>%
  stringr::str_flatten()

# Calculate epsilon in binary format from the gamma, since the epsilon is just has
# the inverted values. 1 turns into 0, and 0 turns into 1
epsilon_binary <- gamma_binary %>%
   stringr::str_replace_all("0", "joker") %>%
      stringr::str_replace_all("1", "0") %>%
      stringr::str_replace_all("joker", "1")

# Finally, having the gamma and epsilon in binary format, we can just transform them
# into decimal format using the strtoi function and multiplying them together.
answer <- purrr::map_dbl(
  list(gamma_binary, epsilon_binary),
  ~ strtoi(.x, base = 2)
  ) %>%
  prod()

# Given that, we can see the answer to first part is: 4,147,524
answer
